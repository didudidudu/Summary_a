	...
	proto.slider0 = function(c,w,n) 
	{
		...
		{			
			...
			setInterval(function()
			{
				if(isDrag)
				{
					p.value = Math.round(_hit.y/_w*_n);
				}else
				{
					if(_hit.y != _block.y)
					{
		...
		{		
			...
			for(var i = 1; i<_n;i++)
			{
				var d = kd.line(0,-i*u,_h*0.8,-i*u);
			}
			var bottom = kd.line(0,0,0,-_w);
			
			kd.line(0,-_lsw*0.5,_h,-_lsw*0.5);
			kd.line(0,-_w+_lsw*0.5,_h,-_w+_lsw*0.5);
			kd.y = _h/2;
			_block = drawBlock();
			p.selectAll("line").attr({
				stroke: _c,
			...
			bottom.attr({strokeWidth:_bsw});
			//�����飨���أ�
			_hit = p.rect(-23,0,46,41);
			_hit.y = _h/2;
			_hit.attr({opacity:0});
			//_hit.style.cursor="pointer";
		}
		function drawBlock()
		{
			var gb = p.g();				
			//var bg = gb.polyline([-23,41],[-23,19],[0,0],[23,19],[23,41]);
			var bg=gb.circle(0,8,8)
			bg.attr({
			   fill:"#495F85",
			});
					var bg1=gb.circle(0,8,6)
			bg1.attr({
			   fill:"#33CCFF",
			});
//			gb.rect(-7,34,4,4);
